...
# Clean dark theme, crisp KPIs, consistent Plotly styling, downloads, and robust ML.

# ---------------------------
# Safety net: install missing libs locally if needed
# ---------------------------
def _ensure(pkgs):
...
# Dark palette + fixed alignment for KPI cards
st.markdown("""
<style>
:root{
  --bg:#0f1116;
...
.kpi-card .label{ color:var(--muted); font-size:.95rem; letter-spacing:.01em; margin-bottom:6px; }
.kpi-card .value{ font-size:1.9rem; font-weight:800; line-height:1.1; color:var(--text); opacity:1!important }

/* Ensure headings/markdown are not dimmed */
h1,h2,h3,h4,h5,h6, .stMarkdown{color:var(--text)!important; opacity:1!important}
...
with tab_overview:
    ...
    with c1:
        ...
        <div class="kpi-card">
          ...
          <div class="value">{raw['Symbol'].nunique():,}</div>
    ...
    with c3:
        ...
        <div class="kpi-card">
          ...
          <div class="value">{raw['Date'].min().date()} → {raw['Date'].max().date()}</div>
        </div>""", unsafe_allow_html=True)
    with c4:
        st.markdown(f"""
        <div class="kpi-card">
...
with tab_bt:
    ...
    if bt is None or probas is None:
        ...
        for name, proba in probas.items():
            if len(proba) != len(bt):  # defensive alignment
...
- **Interpretability**: feature correlations (+ RF importances if added) to justify signals.
